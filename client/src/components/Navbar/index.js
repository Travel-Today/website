...
export default function Navbar({ isLogged, setLoggedIn }) {
  ...
  return (
    <Disclosure
      as="nav"
      className="bg-slate-800 bg-opacity-95 bg-clip-padding blur-backdrop-filter fixed z-10 w-full "
    >
      {({ open }) => (
        <>
            ...
            <div className="relative flex items-center justify-between h-16">
              ...
              <div className="flex-1 flex items-center justify-center sm:items-stretch sm:justify-start">
                <div className="flex-shrink-0 flex items-center">
                  <Link to="/">
                    <h6 className="hidden md:block text-white text-2xl">
                      TRAVEL TODAY
                    </h6>
                  </Link>
                </div>
...
const ProfileMenu = (isLogged, setLoggedIn) => {
  ...
  return (
    <div className="absolute inset-y-0 right-0 flex items-center pr-2 sm:static sm:inset-auto sm:ml-6 sm:pr-0">
        ...
        <LoginButton />
      <a
        href="https://dev-lysw2jct.us.auth0.com/u/signup?state=hKFo2SBCRTM1NExsTWxhcTVPbFowUXVkSlJGRzJHNXVCbkpYUKFur3VuaXZlcnNhbC1sb2dpbqN0aWTZIE1wcTZMblUwZmVDLUktcENrdVhyWjJDYUszSHRWM29io2NpZNkgNFNvbndoZGlNeEFFajNtUkgyMVVWTmNwTHNaYlZybjI"
        className="py-2 px-4 uppercase text-xs text-blue-900 font-bold bg-white rounded"
      >
        Sign up
      </a>
    </div>
